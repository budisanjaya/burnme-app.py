...
# Ventilation Rate = PBL * WindSpeed
...
with col1:
    ...
    if vr is not None:
        st.write(f"**Ventilation Rate (VR):** {vr:.2f} m²/s")
    else:
        st.write("**Ventilation Rate (VR):** Tidak tersedia")
...
# 6. Ventilation Rate 3580–44720 m²/s
if vr is None or not aman_di_rentang(vr, 3580, 44720, include_low=True, include_high=True):
    aman = False
    if vr is not None:
        alasan.append(
            f"Ventilation Rate {vr:.2f} m²/s di luar rentang aman 3580–44720 m²/s."
        )
    else:
        alasan.append("Ventilation Rate tidak tersedia.")
